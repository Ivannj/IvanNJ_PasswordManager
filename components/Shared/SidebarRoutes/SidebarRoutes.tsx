          ...
          <AccordionItem value="item-1" className="border-b-0">
            ...
            <AccordionContent>
              {children.map(({ item, href, icon: Icon }) => (
                <div key={item}>
                  <Link href={href} className="px-6 py-2 flex gap-2 items-center hover:bg-violet-100/20 duration-300 transition-all rounded-md">
                    <Icon size={20} />
                    {item}
                  </Link>
                </div>
          ...
          <AccordionItem value="item-1" className="border-b-0">
            ...
            <AccordionContent>
              {children.map(({ item, href, icon: Icon, premium }) => (
                <div key={item} className="flex items-center justify-between mt-2 hover:bg-violet-100/20 duration-300 transition-all rounded-md pr-1">
                  <Link href={href} className="px-6 flex gap-2 items-center">
                    <Icon size={20} />
                    {item}
                  </Link>
                  {premium && (
                    <span className="flex gap-2 text-xs px-2 py-1 bg-violet-400 rounded-md">
                      Premium
                    </span>
                  )}
                </div>
